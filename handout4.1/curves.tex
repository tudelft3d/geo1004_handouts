...
When curved geometries are present, they are usually simple parametric shapes, such as spheres, cylinders and cones in CSG or circular arcs in certain 2D datasets (\reffig{fig:circulararc}).
...
In general, curve and surface modelling is done by specifying the locations of points, of which there are two kinds (\reffig{subfig:pointtypesa}):
...
If you have some experience with vector-based graphic editors (\eg\ Adobe Illustrator, Inkscape or Sketch), you have likely drawn curves using data points and control points (\reffig{subfig:pointtypesb}).
...
For instance, in the curve \(C(x,y) = (\cos t, \sin t)\) with \(0 \leq t \leq 2\pi \) (\reffig{fig:derivatives}), which describes the unit circle, at \(t=0\) we get the point \((1, 0)\), \ie\ the rightmost point on the circle using the typical axis directions.
...
Most commonly, the data points will be used for this purpose, which for uniform parametric curves will be at \(t = 0, 1, 2, \ldots \) (\reffig{fig:segments}).
...
Similarly, a set of two curves with fixed \(u\) and two curves with fixed \(v\) will bound a patch, \eg\ \(S(u, 0)\), \(S(u, 1)\), \(S(0, v)\) and \(S(1, v)\) (\reffig{fig:patch}).
...
\begin{description}
\item[Positional] (\(G^0\)) continuity means that the boundary of a segment or patch matches that of its neighbours, \ie\ there are no holes at common boundaries (\reffig{subfig:g0});
\item[Tangential] (\(G^1\)) means that the angles of segments or patches match those of its neighbours at their common boundaries, \ie\ no sharp edges at common boundaries (\reffig{subfig:g1});
...
The most common forms of B\'ezier curves are however quadratic B\'ezier curves (\reffig{fig:quadratic}), which have one intermediate point, \eg\ \(p_{\mathrm{quadratic}} = (p_0, p_1, p_2)\), and cubic B\'ezier curves (\reffig{fig:cubic}), which have two intermediate points, \eg\ \(p_{\mathrm{cubic}} = (p_0, p_1, p_2, p_3)\).
...
The exact functions used to determine the weights in B\'ezier curves are called \emph{Bernstein polynomials} (\reffig{fig:bernstein}), named after Sergei Natanovich Bernstein who discovered them in the 1910s.
These are given by:
...
\(\binom{n}{i}\) is the \emph{binomial coefficient}, which is equivalent to the \(i\)-th column of the \(n\)-th row in Pascal's triangle (\reffig{fig:pascal}).
...
\reffig{fig:pointtypes} also shows how these commonly look in software, where the intermediate control points are shown as `handles' around the data points.
...
Therefore, \(G_1\) continuity can be achieved by making sure that the common endpoint and its two neighbours, \ie\ \(p_{n-1}\) and \(q_1\), are collinear (\reffig{fig:polybezier}).
...
The most common are biquadratic (\(3 \times 3\); \reffig{fig:quadraticsurf}) and bicubic (\(4 \times 4\); \reffig{fig:cubicsurf}) surfaces, which are defined based on square matrices of points, such as:
...
These are better parametrised in terms of three barycentric coordinates, which we here denote as \(u\), \(v\) and \(w\) (\reffig{fig:triangle}).
...
% Spline curves, named after flat splines (\reffig{fig:spline}), refer to a variety of piecewise polynomial functions that connect smoothly at certain data points.
